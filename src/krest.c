...
internal void
Win32IoCallbackThread(PTP_CALLBACK_INSTANCE Instance, PVOID Context, PVOID POverlapped, ULONG IoResult, ULONG_PTR BytesTransferred, PTP_IO IoThreadpool)
{
...
// TODO(kstandbridge): Is this the threadpool creation?
internal b32
Win32InitializeIoThreadPool(server_context *ServerContext)
{
    ...
    u32 Result = Win32HttpCreateRequestQueue(ServerContext->Version, L"KrestHttpServer", 0, 0, &ServerContext->RequestQueue);
    if(Result == NO_ERROR)
    {
...
ProcessReceiveAndPostResponse(http_io_request *IoRequest, PTP_IO IoThreadpool, u32 IoResult)
{
    ...
    if(IoResponse != 0)
    {
           ...
           (Result != ERROR_IO_PENDING))
        {
            ...
            // TODO(kstandbridge): Clean up allocated memory for IoRequest?
            // CleanupHttpIoResponse(IoResponse);
        }
    }
}
...
PostNewReceive(server_context *ServerContext, PTP_IO IoThreadpool)
{
    ...
    if(IoRequest != 0)
    {
           ...
           (Result != NO_ERROR))
        {
            ...
            // TODO(kstandbridge): Clean up allocated memory for IoRequest?
            // CleanupHttpIoRequest(IoRequest);
        }
        else
        {
            LogVerbose("HttpReceiveHttpRequest result code %u", Result);
        }
    }
    else
    {
...
#define REQUESTS_PER_PROCESSOR 4
    ...
    // TODO(kstandbridge): Start server
